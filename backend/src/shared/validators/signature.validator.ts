...
export class SignatureValidator {
   ...
   */
  static isValidBase64(signature: string): boolean {
   ...
   */
  static getBase64SizeInBytes(signature: string): number {
    if (!signature || signature.length === 0) {
      return 0;
    }
   ...
   */
  static getBase64SizeInMB(signature: string): number {
   ...
   */
  static isWithinSizeLimit(signature: string): boolean {
    const bytes = this.getBase64SizeInBytes(signature);
   ...
   * }
   */
  static validate(signature: string): { valid: boolean; error?: string } {
   ...
   */
  static isValidImageUrl(signature: string): boolean {
   ...
   */
  static isValidSignatureOrUrl(signature: string): boolean {
